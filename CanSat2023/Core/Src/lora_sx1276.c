...
void lora_mode_receive_continuous(lora_sx1276 *lora)
{
  ...
  set_mode(lora, OPMODE_RX_CONTINUOUS | OPMODE_LONG_RANGE_MODE);
}
...
static uint8_t lora_send_packet_base(lora_sx1276 *lora, uint8_t *data, uint8_t data_len, uint8_t mode)
{
  ...
  set_mode(lora, OPMODE_TX | OPMODE_LONG_RANGE_MODE);
  return LORA_OK;
}
    ...
    uint16_t nss_pin, uint64_t freq)
{
  ...
  lora_set_spreading_factor(lora, 9); //12
  ...
  write_register(lora, REG_MODEM_CONFIG_3, 0x0C);
  ...
  */
  /*
  lora_set_frequency(lora, freq);
  lora_set_spreading_factor(lora, 9); //9
  //lora_set_preamble_length(lora, 6);
  // By default - explicit header mode
  lora_set_explicit_header_mode(lora);
  // Set LNA boost
