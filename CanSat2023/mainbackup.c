  ...
  for(;;){
	 ...
	 */
	 ...
	 //raw = HAL_ADC_GetValue(&hadc1);
	 		...
	 		memcpy(packet,packet1,sizeof(packet1));
	 		memset(str, 0, sizeof(str));
...
void StartTask03(void const * argument)
{
  ...
  /* Infinite loop */
	  ...
	  for(;;)
	  {
		 ...
		 counterPackInPage = (counterPackInPage+1)%3;
		 if (counterPage % 16 == 0) W25qxx_EraseSector((uint32_t)counterPage/16);
		 if (counterPackInPage == 0){
			 W25qxx_WritePage(flashBlock, counterPage, 0, PACKET_LEN*3);
			 memset(flashBlock, 0, sizeof(flashBlock));
			 W25qxx_ReadPage(flashBlock, counterPage, 0, PACKET_LEN*3);
			 CDC_Transmit_FS(flashBlock, PACKET_LEN*3);
			 counterPage += 1;
		 }
